...
const PropertyCard : React.FC<PropertyProps> = (property)=>{
    ...
    return(
        <Link href={`/property/${property.name}`} className="no-underline">
            <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 cursor-pointer">
                <div className="relative w-full h-48 overflow-hidden">
                    <Image src={property.image}  alt={property.name} fill
                ...
                <div className="p-4">
                    ...
                    {property.address && (<p className="text-sm text-gray-600 mb-2">{property.address.city}, {property.address.country} </p>)}
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {property.discount? <span>⬇️ <span className="text-green-600 font-bold">{property.discount}%</span></span> : ""}
                    </div>
                </div>
            </div>
        </Link>
